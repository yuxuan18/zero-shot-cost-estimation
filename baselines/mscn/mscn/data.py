...
def load_data(file_name, num_materialized_samples):
    ...
    for i in range(len(tables)):
        samples.append(np.ones((num_materialized_samples, 1), dtype=np.uint8))  # Placeholder for bitmaps
...
def load_and_encode_train_data(num_queries, num_materialized_samples, enable_stats=True):
    file_name_queries = "data/tpcds"
    file_name_column_min_max_vals = "data/column_min_max_vals_tpcds.csv"
