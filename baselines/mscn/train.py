...
def print_qerror(preds_unnorm, labels_unnorm):
    ...
    return np.median(qerror)


def save_model(model, workload_name):
    model_file = "models/mscn_" + workload_name + ".pt"
    os.makedirs(os.path.dirname(model_file), exist_ok=True)
    torch.save(model.state_dict(), model_file)
    print("Model saved to {}".format(model_file))

def load_model(model, workload_name):
    model_file = "models/mscn_" + workload_name + ".pt"
    if os.path.exists(model_file):
        model.load_state_dict(torch.load(model_file))
        print("Model loaded from {}".format(model_file))
    else:
        print("No pre-trained model found at {}".format(model_file))

def test(model, workload_name, num_materialized_samples, batch_size, cuda, table2vec, column2vec, op2vec, join2vec, column_min_max_vals, min_val, max_val):
    # Load test data
    ...
    preds_test_unnorm = np.reshape(preds_test_unnorm, (-1,))
    ...
    with open(file_name, "w") as f:
        for i in range(len(preds_test_unnorm)):
            f.write(label[i] + "," + str(preds_test_unnorm[i]) + "\n")

def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda, test_only=False):
    ...
    if test_only:
        load_model(model, workload_name)
        print("Testing model...")
        test(model, workload_name, num_materialized_samples, batch_size, cuda, table2vec, column2vec, op2vec, join2vec, column_min_max_vals, min_val, max_val)
        return
    ...
    for epoch in range(num_epochs):
        ...
        preds_test_unnorm = np.reshape(preds_test_unnorm, (-1,))
        valid_qerror = print_qerror(preds_test_unnorm, labels_test_unnorm)
        if valid_qerror < best_qerror:
            best_qerror = valid_qerror
            epochs_with_no_improvement = 0
            print("New best Q-Error: {}".format(best_qerror))
            save_model(model, workload_name)
        else:
            epochs_with_no_improvement += 1

        print("Epoch {}, loss: {}, nepochs_wo_improvement {}".format(epoch, loss_total / len(train_data_loader), epochs_with_no_improvement))

        if epochs_with_no_improvement >= 20:
            print("Early stopping triggered after {} epochs without improvement.".format(epochs_with_no_improvement))
            break
    ...
    # preds_train_unnorm = np.reshape(preds_train_unnorm, (-1,))
...
def main():
    ...
    train_and_predict(args.testset, args.queries, args.epochs, args.batch, args.hid, args.cuda, args.test_only)
