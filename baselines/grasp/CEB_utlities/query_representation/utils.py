...
# def extract_join_clause(query):
    ...
    # for match in matches:
        ...
        # if "!" in match:
            ...
            # if "." in right:
                # # must be a joinextract_join_graph, so add it.
...
# sql1 = "SELECT COUNT(*) FROM keyword as k,title as t,kind_type as kt,info_type as it1,movie_info as mi1,movie_info as mi2,cast_info as ci,role_type as rt,name as n,movie_keyword as mk WHERE mi1.movie_id = t.id AND t.id = mi2.movie_id AND mi2.movie_id = mk.movie_id AND mk.movie_id = ci.movie_id AND k.id = mk.keyword_id AND mi1.info_type_id = it1.id AND kt.id = t.kind_id AND ci.person_id = n.id AND rt.id = ci.role_id AND (t.production_year <= 2015) AND (t.production_year >= 1990) AND (kt.kind in ('tv movie','tv series','video game')) AND (it1.id in ('16')) AND (mi1.info in ('USA:2006','USA:2007','USA:2008','USA:2009','USA:2010')) AND (mi2.info in ('English')) AND (rt.role in ('costume designer')) AND (n.gender in ('f'));"
# sql2 = "SELECT COUNT(*) FROM kind_type as kt;"

# join_graph = extract_join_graph(sql1)
#
# predicates = []
# pred_cols = []
# pred_types = []
# pred_vals = []
# for node in join_graph.nodes(data=True):
#     info = node[1]
#     print(info)
