...
def calculate_proxy_error(training_embeddings, test_embeddings, training_labels, test_estimations, training_estimations):
    ...
    top_indices = np.argsort(similarity_matrix, axis=1)[:, -3:]
...
def calculate_threshold(proxy_q_error, true_q_error, 
    ...
    threshold = calculate_threshold_helper(proxy_q_error, true_q_error, q_error_cutoff, recall_target)
    ...
    corrected_threshold = calculate_threshold_helper(proxy_q_error, true_q_error, q_error_cutoff, corrected_recall)
...
if __name__ == "__main__":
    ...
    np.random.seed(233)
    all_embeddings = np.load("data/tpch/train_embeddings.npy")
    all_labels, all_predictions = read_results("data/tpch/train_predictions.csv")
    ...
    threshold = calculate_threshold(proxy_q_error, true_q_error, 
                                    confidence_level=0.95, q_error_cutoff=20, recall_target=0.9)
    ...
    eval_embeddings = np.load("data/tpch/eval_embeddings.npy")
    eval_labels, eval_predictions = read_results("data/tpch/eval_predictions.csv")
    ...
    with open("data/tpch/confidence.csv", "w") as f:
